...
void CScene::SetCPhysicsPtr(CPhysics* lpPhysics) {
	...
	m_lpDefaultSphere = m_lpPhysics->GetListDefaultSpherePtr();
...
void CScene::InitD3D(UINT width, UINT height) {
	...
	Arrow.SetLogBookPtr(m_lpLogBook);
	Arrow.SetD3Device(m_lpD3DDevice);
	Arrow.SetResourceManagerPtr(lpResmanager);
	Arrow.Init();

	m_lpLogBook->Succed(L"Arrow-Klasse initialisiert");
...
void CScene::DeInitD3D() {
	...
	Arrow.DeInit();
	m_lpLogBook->Succed(L"Arrow-Klasse wurde beendet");
...
void CScene::Render(void) {
	...
	for(itOE = m_lpListEmitter->begin(); itOE != m_lpListEmitter->end(); itOE++) {
		...
		//EmitterObject.SetRotation((*itOE)->
	...
	for(itP = m_lpListProtone->begin(); itP!= m_lpListProtone->end(); itP++ ) {
		...
		if(Selected == (LPDEFAULTELEMENT)(*itP)) {
			if(SelectedPart == SELECTED_SPEED) Arrow.SetPosition(((*itP)->Position + (*itP)->Speed * FACTOR_SPEED_ARROW));
			else if(SelectedPart == SELECTED_SPEEDUP) Arrow.SetPosition(((*itP)->Position + (*itP)->SpeedingUp * FACTOR_SPEEDUP_ARROW));
	...
	for(itE = m_lpListElectrone->begin(); itE!= m_lpListElectrone->end(); itE++ ) {
		...
		if(Selected == (LPDEFAULTELEMENT)(*itE)) {
			if(SelectedPart == SELECTED_SPEED) Arrow.SetPosition(((*itE)->Position + (*itE)->Speed * FACTOR_SPEED_ARROW));
			else if(SelectedPart == SELECTED_SPEEDUP) Arrow.SetPosition(((*itE)->Position + (*itE)->SpeedingUp * FACTOR_SPEEDUP_ARROW));
	...
	if(ShowArrow && bArrowActivated){
		m_lpD3DDevice->Clear(0, NULL, D3DCLEAR_ZBUFFER, NULL, 1.0f, 0);

		Arrow.Render(CameraPosition);
	}

	/*WCHAR Text[256];
	StringCchPrintf( Text, 256, L"Width: %f  Height: %f ", CameraRotation.x, CameraRotation.y);

	DrawText(Text, 20, 20, 0xFF000000);*/
...
BOOL CScene::Pick(BOOL bAddSelection){
	...
	if(ShowArrow && bArrowActivated) {
		if(IsPicked(Arrow.Get3DObject(0))) {
			ArrowSelected[0] = true;
			return true;
		} else if(IsPicked(Arrow.Get3DObject(1))) {
			ArrowSelected[1] = true;
			return true;
		} else if(IsPicked(Arrow.Get3DObject(2))) {
	...
	for(itP = m_lpListProtone->begin(); itP!= m_lpListProtone->end(); itP++ ) {
		if((*itP)->bShowSpeed){
			...
			if(IsPicked(VectorArrow.Get3DObject(ARROW_SPEED))) {
				...
				Arrow.SetPosition((*itP)->Position + (*itP)->Speed * FACTOR_SPEED_ARROW);
				return true;
			}
		}
	}
	...
	for(itE = m_lpListElectrone->begin(); itE!= m_lpListElectrone->end(); itE++ ) {
		if((*itE)->bShowSpeed){
			...
			if(IsPicked(VectorArrow.Get3DObject(ARROW_SPEED))) {
				...
				SelectedPart = SELECTED_SPEED;
				Arrow.SetPosition((*itE)->Position + (*itE)->Speed * FACTOR_SPEED_ARROW);
				return true;
			}
		}
	}
	...
	for(itP = m_lpListProtone->begin(); itP!= m_lpListProtone->end(); itP++ ) {
		if((*itP)->bShowSpeedup){
			...
			if(IsPicked(VectorArrow.Get3DObject(ARROW_SPEEDUP))) {
				...
				Arrow.SetPosition((*itP)->Position + (*itP)->SpeedingUp * FACTOR_SPEEDUP_ARROW);
				return true;
			}
		}
	}
	...
	for(itE = m_lpListElectrone->begin(); itE!= m_lpListElectrone->end(); itE++ ) {
		if((*itE)->bShowSpeedup){
			...
			if(IsPicked(VectorArrow.Get3DObject(ARROW_SPEEDUP))) {
				...
				Selected = (LPDEFAULTELEMENT) (*itE);
				SelectedPart = SELECTED_SPEEDUP;
				Arrow.SetPosition((*itE)->Position + (*itE)->SpeedingUp * FACTOR_SPEEDUP_ARROW);
				return true;
			}
		}
	}
	...
	for(itEF = m_lpListElectricalField->begin(); itEF!= m_lpListElectricalField->end(); itEF++ ) {
		if((*itEF)->bShowVector) {
			...
			if(IsPicked(VectorArrow.Get3DObject(ARROW_SPEEDUP))) {
				...
				Arrow.SetPosition((*itEF)->Position + (*itEF)->ViewAt  * 1.0f);
				return true;
			}
		}
	}
	...
	for(itMF = m_lpListMagneticalField->begin(); itMF!= m_lpListMagneticalField->end(); itMF++ ) {
		if((*itMF)->bShowVector) {
			...
			if(IsPicked(VectorArrow.Get3DObject(ARROW_SPEEDUP))) {
				...
				SelectedPart = SELECTED_FORCE;
				Arrow.SetPosition((*itMF)->Position + (*itMF)->ViewAt  * 1.0f);
				return true;
			}
		}
	}
	...
	for(itGF = m_lpListGravityField->begin(); itGF!= m_lpListGravityField->end(); itGF++ ) {
		if((*itGF)->bShowVector) {
			...
			if(IsPicked(VectorArrow.Get3DObject(ARROW_SPEEDUP))) {
				...
				SelectedPart = SELECTED_FORCE;
				Arrow.SetPosition((*itGF)->Position + (*itGF)->ViewAt  * 1.0f);
				return true;
			}
		}
	}
	...
	for(itOE = m_lpListEmitter->begin(); itOE!= m_lpListEmitter->end(); itOE++ ) {
		if((*itOE)->bShowVector) {
			...
			if(IsPicked(VectorArrow.Get3DObject(ARROW_SPEEDUP))) {
				...
				Arrow.SetPosition((*itOE)->Position + (*itOE)->ViewAt  * 1.0f);
				return true;
			}
		}
	}
	...
	for(itP = m_lpListProtone->begin(); itP!= m_lpListProtone->end(); itP++ ) {
		...
		if(IsPicked(lpD3Object)) {
			...
			if(!bAddSelection) {
				Arrow.SetPosition((*itP)->Position);
				return true;
			}
		}
	}	
	...
	for(itEF = m_lpListElectricalField->begin(); itEF!= m_lpListElectricalField->end(); itEF++ ) {
		...
		if(IsPicked(lpD3Object)) {
			...
			Selected = (LPDEFAULTELEMENT) (*itEF);
			if(!bAddSelection) {
				Arrow.SetPosition((*itEF)->Position);
				return true;
			}
		}
	...
	for(itMF = m_lpListMagneticalField->begin(); itMF!= m_lpListMagneticalField->end(); itMF++ ) {
		...
		if(IsPicked(lpD3Object)) {
			...
			Selected = (LPDEFAULTELEMENT) (*itMF);
			if(!bAddSelection) {
				Arrow.SetPosition((*itMF)->Position);
				return true;
			}
		}
	...
	for(itGF = m_lpListGravityField->begin(); itGF!= m_lpListGravityField->end(); itGF++ ) {
		...
		if(IsPicked(lpD3Object)) {
			...
			Selected = (LPDEFAULTELEMENT) (*itGF);
			if(!bAddSelection) {
				Arrow.SetPosition((*itGF)->Position);
				return true;
			}
		}
	}
	...
	for(itOE = m_lpListEmitter->begin(); itOE!= m_lpListEmitter->end(); itOE++ ) {
		...
		if(IsPicked(EmitterObject.GetMeshPtr())) {
			...
			Selected = (LPDEFAULTELEMENT) (*itOE);
			if(!bAddSelection) {
				Arrow.SetPosition((*itOE)->Position);
				return true;
			}
		}
	}
...
//	D3DSURFACE_DESC* _pd3dsdBackBuffer;
	...
	switch(msg) {
		...
		case WM_MOUSEMOVE:
			if(IsButtonDown[2] ) {
				...
				dis *= Arrow.GetDistance();

				
				Arrow.AddPosition(D3DXVECTOR3(dis, 0.0f, 0.0f));
				if(SelectedPart == SELECTED_ELEMENT){
					for(itP = m_lpListProtone->begin(); itP != m_lpListProtone->end(); itP++) {
				...
				else if(SelectedPart == SELECTED_SPEED) ((Electrone*)Selected)->Speed = ( Arrow.GetPosition() - ((Electrone*)Selected)->Position) / FACTOR_SPEED_ARROW;
				else if(SelectedPart == SELECTED_SPEEDUP) ((Electrone*)Selected)->SpeedingUp = ( Arrow.GetPosition() - ((Electrone*)Selected)->Position) / FACTOR_SPEEDUP_ARROW;
				else if(SelectedPart == SELECTED_FORCE) ((ElectricalField*)Selected)->ViewAt = Arrow.GetPosition() - ((ElectricalField*)Selected)->Position;
				else if(SelectedPart == SELECTED_DIRECTION) ((Emitter*)Selected)->ViewAt = Arrow.GetPosition() - ((Emitter*)Selected)->Position;
				m_OnArrowMoved();
				...
				Arrow.AddPosition(D3DXVECTOR3(0.0f, dis, 0.0f));
				if(SelectedPart == SELECTED_ELEMENT){
					for(itP = m_lpListProtone->begin(); itP != m_lpListProtone->end(); itP++) {
				...
				Arrow.AddPosition(D3DXVECTOR3(0.0f, 0.0f, dis));
				if(SelectedPart == SELECTED_ELEMENT){
					for(itP = m_lpListProtone->begin(); itP != m_lpListProtone->end(); itP++) {
